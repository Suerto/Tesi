...
La natura esplorativa del progetto ha richiesto inoltre una costante attività di auto-formazione. Ho supportato ogni fase di implementazione mediante la consultazione sistematica delle documentazioni ufficiali delle tecnologie adottate: la guida di riferimento di React, il portale di Microsoft Learn per l'ecosistema .NET e la documentazione tecnica di Informix. Tale pratica non ha rappresentato un mero supporto all'implementazione, ma un'attività di analisi tecnica strutturata per comprendere le \textit{best practice} di ogni \textit{framework} e applicarle correttamente nel contesto aziendale.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/design1.jpeg}
    \vspace{0.3cm}
    \includegraphics[width=1.0\textwidth]{img/design2.jpeg}
...
Sul livello \textit{frontend} in React e TypeScript, ho configurato un ambiente di analisi statica (\textit{linting}) tramite ESLint e Prettier. Infine, per preservare le prestazioni, ho implementato un gestore universale delle eccezioni --- evitando la duplicazione di costrutti \texttt{try-catch} in violazione del principio \gls{dry} --- e un \gls{rate-limiting} sulle API, una tecnica di controllo del traffico che limita il numero di richieste che un client può inviare a un servizio in un determinato intervallo di tempo, prevenendo così tentativi di abuso o il sovraccarico involontario dei \textit{server}.
